...
TempMotifCounter::TempMotifCounter(const PNGraph& static_graph, const TVec< THash<TInt, TIntV> >& temporal_data) {
  static_graph_ = static_graph;
  temporal_data_ = temporal_data;
  wt_ = Identity();
}
...
void TempMotifCounter::Count3TEdge2Node(int u, int v, double delta,
  ...
  counter.Count(in_out, timestamps, delta, counts, wt_);
}
  ...
  for (int c = 0; c < centers.Len(); c++) {
    ...
    for (int i = 0; i < nbrs.Len(); i++) {
      for (int j = i + 1; j < nbrs.Len(); j++) {
        ...
        Counter3D local;
        counter.Count(edge_id, timestamps, delta, local, wt_);
...
void TempMotifCounter::Count3TEdge3NodeStars(double delta, Counter3D& pre_counts,
  ...
  for (int c = 0; c < centers.Len(); c++) {
    ...
    tesc.Count(ordered_events, timestamps, delta, wt_);
...
void TempMotifCounter::Count3TEdgeTriads(double delta, Counter3D& counts) {
  ...
  for (int edge_id = 0; edge_id < all_edges.Len(); edge_id++) {
    ...
    tetc.Count(sorted_events, timestamps, delta, wt_);
    #pragma omp critical
    {
      for (int dir1 = 0; dir1 < 2; dir1++) {
        for (int dir2 = 0; dir2 < 2; dir2++) {
